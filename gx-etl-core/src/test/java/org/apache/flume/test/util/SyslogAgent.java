...
/**
 * Syslog Flume Agent. A Syslog source of some kind is configured and a client
 * is available to write messages to the agent. The Flume agents port is
 * randomly assigned (not in use).
 *
 */
		...
		while (client == null) {
			try {
				...
				if (++numberOfAttempts >= DEFAULT_ATTEMPTS) {
					throw new AssertionError("Could not connect to source after " + DEFAULT_ATTEMPTS + " attempts with "
							+ DEFAULT_TIMEOUT + " ms timeout.");
				}
