...
package com.geostax.etl.serialization;
...
/**
 * <p/>
 * Class that stores object state in an avro container file. The file is only
 * ever appended to. At construction time, the object reads data from the end of
 * the file and caches that data for use by a client application. After
 * construction, reads never go to disk. Writes always flush to disk.
 *
 * <p/>
...
public class DurablePositionTracker implements PositionTracker {
	 ...
	 * file from a previous crash. If we append to a bad record, our writes may
	 * never be visible.
	 * 
	 * @param trackerFile
	 * @param target
	...
	DurablePositionTracker(File trackerFile, String target) throws IOException {
		...
		DatumWriter<TransferStateFileMeta> dout = new SpecificDatumWriter<TransferStateFileMeta>(
				TransferStateFileMeta.SCHEMA$);

		DatumReader<TransferStateFileMeta> din = new SpecificDatumReader<TransferStateFileMeta>(
				TransferStateFileMeta.SCHEMA$);
