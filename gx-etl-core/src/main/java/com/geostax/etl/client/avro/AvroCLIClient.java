...
public class AvroCLIClient {
	...
	private boolean parseCommandLine(String[] args) throws ParseException {
		...
		options.addOption("P", "rpcProps", true, "RPC client properties file with " + "server connection params")
				...
				.addOption("R", "headerFile", true,
						("file containing headers as " + "key/value pairs on each new line"))
		...
		if (commandLine.hasOption("rpcProps")) {
			...
			Preconditions.checkNotNull(rpcClientPropsFile,
					"RPC client properties " + "file must be specified after --rpcProps argument.");
	...
	private void run() throws IOException, FlumeException, EventDeliveryException {
		...
		try {
			if (fileName != null) {
				...
				reader = new ReliableSpoolingFileEventReader.Builder().spoolDirectory(new File(dirName)).build();
			} else {
