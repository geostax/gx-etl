...
import com.geostax.etl.lifecycle.LifecycleSupervisor;
import com.geostax.etl.lifecycle.LifecycleSupervisor.SupervisorPolicy;
 ...
 * applications. This Agent is mean to be much simpler than a traditional agent
 * and as such it's more restrictive than what can be configured for a
 * traditional agent. For specifics see the Flume User Guide.
 */
...
public class EmbeddedAgent {
	...
	/**
	 * Configures the embedded agent. Can only be called after the object is created
	 * or after the stop() method is called.
	 *
	 * @param properties
	 ...
	 *             if called while the agent is started
	 */
	 ...
	 * @throws IllegalStateException
	 *             if the agent has not been configured or is already started
	 */
	...
	/**
	 * Stops the agent. Can only be called after a successful call to start(). After
	 * a call to stop(), the agent can be re-configured with the configure() method
	 * or re-started with the start() method.
	 *
	 * @throws FlumeException
	...
	/**
	 * Adds event to the channel owned by the agent. Note however, that the event is
	 * not copied and as such, the byte array and headers cannot be re-used by the
	 * caller.
	 * 
	 * @param event
	...
	public void put(Event event) throws EventDeliveryException {
		...
		try {
			...
			throw new EventDeliveryException("Embedded agent " + name + ": Unable to process event: " + ex.getMessage(),
					ex);
		}
	}

	/**
	 * Adds events to the channel owned by the agent. Note however, that the event
	 * is not copied and as such, the byte array and headers cannot be re-used by
	 * the caller.
	 * 
	 * @param events
	 * @throws EventDeliveryException
